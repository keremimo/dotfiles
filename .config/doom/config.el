...
(map! :leader
      :desc "Find file in project" "s f" #'projectile-find-file
      :desc "Search project"       "s s" #'+default/search-project)
...
(after! centaur-tabs
  (dolist (event '(mouse-4 mouse-5 wheel-up wheel-down))
    (define-key centaur-tabs-mode-map
                (vector centaur-tabs-display-line event)
                #'ignore)))
