...
Item {
  ...
  function syncWallpaperColorsOnApplyEnabled() {
    return !!(cfg.applyWallpaperColorsOnApply ?? defaults.applyWallpaperColorsOnApply ?? false);
  }

  function wallpaperColorOptionsForScreen(screenName) {
    const screenConfig = cfg.screens?.[screenName] || ({});
    return {
      "screenName": screenName,
      "scaling": String(screenConfig.scaling || defaultScaling || "fill")
    };
  }
  ...
  IpcHandler {
    ...
    function apply(screenName: string, bgPath: string) {
      ...
      if (root.syncWallpaperColorsOnApplyEnabled()) {
        root.scheduleWallpaperColorsFromPath(bgPath, root.wallpaperColorOptionsForScreen(screenName));
      }
    }
